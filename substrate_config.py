...
class SubstrateConfigManager:
    ...
    def _load_config(self) -> SubstrateConfig:
        ...
        if auto_detect and not (os.environ.get('SUBSTRATE_HTTP_PORT') and os.environ.get('SUBSTRATE_WS_PORT')):
    ...
    def _auto_detect_ports(self) -> Optional[Dict[str, int]]:
    ...
    def _detect_from_process_args(self) -> Optional[Dict[str, int]]:
    ...
    def _detect_from_process_output(self) -> Optional[Dict[str, int]]:
        ...
        try:
                ...
                for line in lines:
                    if 'LISTEN' in line and ('127.0.0.1:' in line or '*:' in line):
                        ...
                        if match:
                            ...
                            if port != 30333 and port != 9615:  # Skip p2p and prometheus ports
                                return {'http': port, 'ws': 9944}
            
            # Method 4: Check recent journalctl logs for this specific process
            journal_result = subprocess.run(
                ['journalctl', '_PID=' + pid, '--since', '5 minutes ago', '--no-pager', '-q'],
            ...
            if journal_result.returncode == 0:
                ...
                rpc_match = re.search(r'Running JSON-RPC server: addr=([^,\s]+)', log_content)
                if rpc_match:
