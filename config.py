...
class SubstrateConfig:
    ...
    ws_port: int = field(default_factory=lambda: int(os.getenv('SUBSTRATE_WS_PORT', '9944')))
    
    # Authentication
    keypair_seed: str = field(default_factory=lambda: os.getenv('SUBSTRATE_KEYPAIR_SEED', '//Alice'))
    
    # Testing
    test_public_key: str = field(default_factory=lambda: os.getenv('SUBSTRATE_TEST_PUBLIC_KEY', '0x1234567890abcdef1234567890abcdef12345678'))
    
    # Node configuration
    node_binary: str = field(default_factory=lambda: os.getenv('SUBSTRATE_NODE_BINARY', 'solochain-template-node'))
    auto_detect: bool = field(default_factory=lambda: os.getenv('SUBSTRATE_AUTO_DETECT', 'true').lower() == 'true')
...
class IPFSConfig:
    ...
    api_url: str = field(default_factory=lambda: os.getenv('IPFS_API_URL', 'http://localhost:5001'))
    gateway_url: str = field(default_factory=lambda: os.getenv('IPFS_GATEWAY_URL', 'http://localhost:8080'))
    
    # Ports
    api_port: int = field(default_factory=lambda: int(os.getenv('IPFS_API_PORT', '5001')))
    gateway_port: int = field(default_factory=lambda: int(os.getenv('IPFS_GATEWAY_PORT', '8080')))
...
class CommuneIPFSConfig:
    ...
    # URLs
    base_url: str = field(default_factory=lambda: f"http://{os.getenv('COMMUNE_IPFS_HOST', 'localhost')}:{os.getenv('COMMUNE_IPFS_PORT', '8000')}")
    
    # IPFS backend configuration
    ipfs_api_url: str = field(default_factory=lambda: os.getenv('COMMUNE_IPFS_API_URL', 'http://localhost:5001'))
    ipfs_gateway_url: str = field(default_factory=lambda: os.getenv('COMMUNE_IPFS_GATEWAY_URL', 'http://localhost:8080'))
...
class TestConfig:
    ...
    test_module_registry_url: str = field(default_factory=lambda: os.getenv('TEST_MODULE_REGISTRY_URL', 'http://localhost:8004'))
    
    # RPC test URLs
    rpc_test_urls: list[str] = field(default_factory=lambda: [
    ...
    substrate_url: str = field(default_factory=lambda: os.getenv('TEST_SUBSTRATE_URL', 'ws://127.0.0.1:9944'))
    ipfs_backend_url: str = field(default_factory=lambda: os.getenv('TEST_IPFS_BACKEND_URL', 'http://localhost:8000'))
...
class SecurityConfig:
    ...
    default_keypair_seed: str = field(default_factory=lambda: os.getenv('DEFAULT_KEYPAIR_SEED', '//Alice'))
    
    # Key generation
    use_secure_random: bool = field(default_factory=lambda: os.getenv('USE_SECURE_RANDOM', 'true').lower() == 'true')
    
    # Environment detection
    is_development: bool = field(default_factory=lambda: os.getenv('ENVIRONMENT', 'development').lower() == 'development')
    is_production: bool = field(default_factory=lambda: os.getenv('ENVIRONMENT', 'development').lower() == 'production')
...
class ModNetConfig:
    ...
    def __post_init__(self):
        ...
        self.commune_ipfs.base_url = f"http://{self.commune_ipfs.host}:{self.commune_ipfs.port}"
    ...
    def print_config(self, show_sensitive: bool = False):
        ...
        print("\n🔒 Security:")
        print(f"   Environment: {'Development' if self.security.is_development else 'Production'}")
    ...
    def validate_config(self) -> list[str]:
        ...
        if self.security.is_production and self.substrate.keypair_seed in ['//Alice', '//Bob', '//Charlie']:
            issues.append("Production environment using development keypair seed")
        
        # Check for localhost in production
        if self.security.is_production:
            if 'localhost' in self.substrate.http_url or '127.0.0.1' in self.substrate.http_url:
                issues.append("Production environment using localhost for Substrate connection")
            if 'localhost' in self.ipfs.api_url or '127.0.0.1' in self.ipfs.api_url:
                issues.append("Production environment using localhost for IPFS connection")
